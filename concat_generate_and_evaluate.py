...
@log_call(logger)
def generate_image(server_url: str, prompt: str, seed: int, output_path: str,
                   timeout: int = 300) -> Dict:
    """
    POST prompt + seed to the local FastAPI generation server and write the
    returned PNG bytes directly to output_path.

    Args:
        server_url  : Full URL of the /generate endpoint,
                      e.g. "http://127.0.0.1:8000/generate".
        prompt      : Text prompt to send to the model.
        seed        : Integer seed that controls the model's stochasticity.
                      Unique seeds across variants produce diverse images for
                      the same prompt (local models are deterministic by default).
        output_path : Local filesystem path where the PNG will be saved.
        timeout     : Seconds to wait for the server to respond.
                      Default 300s because Qwen at 1328x1328 can take ~90s.
    """
    try:
        logger.info(
            f"Requesting image  seed={seed}  "
            f"server={server_url}  "
            f"prompt={prompt[:80]!r}"
        )

        # POST the JSON payload to the server.
        # The server returns raw PNG bytes in the response body.
        ...
        # Raise an HTTPError for 4xx / 5xx status codes so the exception
        # handler below can log and return an error result dict.
        response.raise_for_status()

        # Write the raw PNG bytes to disk without any intermediate decoding.
        with open(output_path, "wb") as f:
            f.write(response.content)

        logger.info(f"✓ Image saved to: {output_path}  ({len(response.content)//1024} KB)")
        ...
        logger.error(
            f"✗ Timeout after {timeout}s waiting for {server_url}. "
            f"Consider increasing --request-timeout."
        )
        return {
            'status': 'error',
...
def generate_images_for_row(server_url: str, row_data: Dict, output_dir: Path, timeout: int = 300) -> List[Dict]:
    """
    Generate all image variants for one CSV row by calling the local server.

    Each element of row_data['precised_prompts'] is a dict:
        {'prompt': str, 'seed': int}
    The unique seed per variant ensures the model produces diverse images
    even when the same text prompt is sent multiple times.

    Note: the time.sleep(1) that existed here for OpenAI rate-limiting has
    been removed. The local server serialises requests internally via its
    asyncio.Lock(), so no client-side throttling is needed.
    """
    results = []
    ...
    log_subsection(logger, f"Row {row_idx}  (CSV ID: {row_id})  ·  {len(precised_prompts)} variants")

    for variant_idx, variant in enumerate(precised_prompts):
        # Each variant is a dict with 'prompt' and 'seed' keys (set in
        # read_simple_prompts).  Unpack both before calling generate_image.
        ...
        result = generate_image(server_url, prompt, seed, str(output_path), timeout=timeout)
...
@log_call(logger)
def process_parallel(server_url: str, prompts_data: List[Dict], output_dir: Path, num_workers: int = 4, timeout: int = 300) -> List[Dict]:
    """
    Process multiple CSV rows concurrently using a thread pool.

    Even though the server processes requests one at a time (asyncio.Lock),
    multiple client threads are still beneficial: while the server is
    generating one image, the other threads are preparing output directories,
    logging, and keeping their HTTP connections open — so the server's queue
    never runs dry between consecutive images.
    """
    all_results = []
    with ThreadPoolExecutor(max_workers=num_workers) as executor:
        future_to_row = {
            executor.submit(generate_images_for_row, server_url, row_data, output_dir, timeout): row_data
...
def generate_images_from_csv(
    ...
    """
    Read prompts from a CSV, generate images via the local server for each,
    ...
    with log_timer(logger, f"[{tag}] full generation pass  ({len(prompts_data)} rows)"):
        if sequential:
            all_results = []
            for row_data in prompts_data:
                results = generate_images_for_row(server_url, row_data, out_dir, timeout=timeout)
                all_results.extend(results)
        else:
            all_results = process_parallel(server_url, prompts_data, out_dir, num_workers=workers, timeout=timeout)
...
def infer_model_tag(server_url: str) -> str:
    lower_url = server_url.lower()
    if "8001" in lower_url or "qwen" in lower_url:
        return "qwen"
    if "8002" in lower_url or "sd35" in lower_url or "sdxl" in lower_url:
        return "sd35"
    if "8003" in lower_url or "zimage" in lower_url or "z-image" in lower_url:
        return "zimage"
    if "8004" in lower_url or "flux_dev" in lower_url or "flux.1-dev" in lower_url:
        return "flux_dev"
    if "8000" in lower_url or "flux" in lower_url:
        return "flux_turbo"
    return "model"
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate 10 images per prompt for ORIGINAL and ENHANCED prompt CSVs, then compute metrics.'
    )
    ...
    parser.add_argument(
        ...
        help=(
            'Skip image generation entirely and run metrics on images that '
            'already exist in --eval-folder.  When this flag is set, all '
            'generation-related arguments (--csv-file, --model-server-url, '
            '--workers, etc.) are ignored.'
        )
    )
    parser.add_argument(
        '--eval-folder', type=str, default=None,
        help=(
            'Root folder containing already-generated images.  '
            'Expected layout (produced by this script):\n'
            '  <folder>/generated_images/original/row_*/  \n'
            '  <folder>/generated_images/enhanced/row_*/  \n'
            'Also accepts the flat layout:\n'
            '  <folder>/original/  and  <folder>/enhanced/\n'
            'Required when --evaluate-only is set.'
        )
    )
    parser.add_argument(
        '--eval-tags', type=str, default='original,enhanced',
        help=(
            "Comma-separated list of tags to evaluate (default: 'original,enhanced'). "
            "Example: --eval-tags original   to run metrics only on the original set."
        )
    )

    # CSV inputs
    parser.add_argument('--csv-file', type=str, default=None, help='Single CSV containing both original & enhanced prompts (overrides --original-csv/--enhanced-csv)')
    
    # Legacy arguments (kept for compatibility, but optional if --csv-file is used)
    parser.add_argument('--original-csv', type=str, default=None, help='CSV containing original prompts')
    parser.add_argument('--enhanced-csv', type=str, default=None, help='CSV containing enhanced prompts')
    
    # Columns
    parser.add_argument('--original-prompt-col', type=str, default='generic_prompt', help="Column for original prompts (default: 'generic_prompt')")
    parser.add_argument('--enhanced-prompt-col', type=str, default='modified_prompts', help="Column for enhanced prompts (default: 'modified_prompts')")
    parser.add_argument('--enhanced-is-json', action=argparse.BooleanOptionalAction, default=True, help="Treat enhanced prompt col as JSON list (default: True)")
    
    parser.add_argument('--num-rows', type=int, default=None, help='Optional: number of rows to process from each CSV (default: all)')
    parser.add_argument('--seed', type=int, default=None, help='Random seed for row selection (default: sequential)')

    # Generation
    parser.add_argument('--images-per-prompt', type=int, default=10, help='Images per prompt (default: 10)')
    parser.add_argument('--output-dir', type=str, default='generated_images', help='Output directory root for images')
    ...
    parser.add_argument(
        '--model-server-url', type=str, default='http://127.0.0.1:8000/generate',
        help=(
            'URL of the local FastAPI image generation server /generate endpoint. '
            'For FLUX.2-dev-Turbo          : http://<host>:8000/generate  (serve_flux.py)  '
            'For Qwen-Image-2512           : http://<host>:8001/generate  (serve_qwen.py)  '
            'For SD 3.5 Large              : http://<host>:8002/generate  (serve_sd35.py)  '
            'For Tongyi-MAI/Z-Image        : http://<host>:8003/generate  (serve_zimage.py)  '
            'For FLUX.1-dev                : http://<host>:8004/generate  (serve_flux_dev.py)  '
    ...
    parser.add_argument(
        ...
        help=(
            'Seconds to wait for the server to return each image (default: 300). '
            'Increase if Qwen at native resolutions is timing out.'
        )
    )

    # Metrics
    parser.add_argument('--metrics-base-dir', type=str, required=True,
                        help='Base dir containing the FairFace and dlib model files (res34_fair_align_multi_7_20190809.pt, shape_predictor_5_face_landmarks.dat)')

    args = parser.parse_args()

    global BASE_DIR, FAIRFACE_MODEL_DIR, DLIB_MODEL_DIR
    # Model files stay in the caller-supplied directory
    FAIRFACE_MODEL_DIR = os.path.abspath(args.metrics_base_dir)
    DLIB_MODEL_DIR = os.path.abspath(args.metrics_base_dir)

    _check_metrics_models(Path(FAIRFACE_MODEL_DIR))

    # -----------------------------------------------------------------------
    # Evaluate-only branch — skip generation, run metrics on existing images
    # -----------------------------------------------------------------------
    if args.evaluate_only:
        if not args.eval_folder:
            parser.error("--eval-folder is required when --evaluate-only is set.")

        eval_folder = Path(args.eval_folder).resolve()
        if not eval_folder.is_dir():
            parser.error(f"--eval-folder does not exist or is not a directory: {eval_folder}")

        # Metrics outputs go into <eval-folder>/eval_outputs/
        BASE_DIR = str(eval_folder / "eval_outputs")
        os.makedirs(BASE_DIR, exist_ok=True)

        tags = [t.strip() for t in args.eval_tags.split(',') if t.strip()]
        ...
        if not evaluated_any:
            logger.error(
                f"No images were found under {eval_folder} for any of the tags {tags}. "
    ...
    # -----------------------------------------------------------------------
    # Normal generation + evaluation path (unchanged below)
    # -----------------------------------------------------------------------

    # Point --model-server-url at whichever server is running:
    #   FLUX.2-dev-Turbo          →  http://<host>:8000/generate  (serve_flux.py)
    #   Qwen-Image-2512           →  http://<host>:8001/generate  (serve_qwen.py)
    #   SD 3.5 Large              →  http://<host>:8002/generate  (serve_sd35.py)
    #   Tongyi-MAI/Z-Image        →  http://<host>:8003/generate  (serve_zimage.py)
    #   FLUX.1-dev                →  http://<host>:8004/generate  (serve_flux_dev.py)
    server_url = args.model_server_url
    model_tag = infer_model_tag(server_url)
    date_tag = datetime.now().strftime('%Y%m%d')
    run_output_tag = f"{model_tag}_output_{date_tag}"

    # All metrics outputs go under evaluation/eval_outputs/<model_output_date>/
    BASE_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'eval_outputs', run_output_tag)
    os.makedirs(BASE_DIR, exist_ok=True)

    logger.info(f"Using local generation server: {server_url}")
    logger.info(f"Run output tag: {run_output_tag}")

    # Determine CSV paths
    if args.csv_file:
        original_csv = args.csv_file
        enhanced_csv = args.csv_file
        logger.info(f"Using single CSV for both passes: {args.csv_file}")
    else:
        if not args.original_csv or not args.enhanced_csv:
            parser.error("Either --csv-file OR both --original-csv and --enhanced-csv must be provided.")
        original_csv = args.original_csv
        enhanced_csv = args.enhanced_csv

    # Generated images go under generated_images/<model_output_date>/
    output_root = Path(args.output_dir) / run_output_tag
    output_root.mkdir(parents=True, exist_ok=True)

    # 1) ORIGINAL prompts
    log_section(logger, "Original prompts  ·  image generation run")
    original_results = generate_images_from_csv(
        server_url=server_url,
        csv_path=original_csv,
        tag="original",
        output_root=output_root,
        prompt_col=args.original_prompt_col,
        is_json_list=False,  # Original prompts are typically single strings
    ...
    if original_img_paths:
        logger.info(f"Running metrics (including ICAD) for ORIGINAL set on {len(original_img_paths)} successfully generated images...")
    ...
    # 2) ENHANCED prompts
    log_section(logger, "Enhanced prompts  ·  image generation run")
    enhanced_results = generate_images_from_csv(
        server_url=server_url,
        csv_path=enhanced_csv,
        tag="enhanced",
        output_root=output_root,
        prompt_col=args.enhanced_prompt_col,
        is_json_list=args.enhanced_is_json,
        num_rows=args.num_rows,
        seed=args.seed,
        images_per_prompt=args.images_per_prompt,
        workers=args.workers,
    ...
    if enhanced_img_paths:
        logger.info(f"Running metrics (including ICAD) for ENHANCED set on {len(enhanced_img_paths)} successfully generated images...")
