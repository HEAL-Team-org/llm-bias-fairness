...
# ── Local LLM servers ───────────────────────────────────────────────────────
# Qwen3-32B  → port 8001  (serve_qwen3.py)
# Qwen3.5-27B → port 8002  (serve_qwen35.py)
MODEL_URLS: dict[str, str] = {
    "qwen3":  "http://localhost:8001",
    "qwen3.5": "http://localhost:8002",
}
# Active URL — changed at runtime by configure_llm() or the --model flag
_ACTIVE_LLM_URL: str = MODEL_URLS["qwen3"]


def configure_llm(model: str) -> None:
    """Point all LLM calls at the server for *model* (``'qwen3'`` or ``'qwen3.5'``)."""
    global _ACTIVE_LLM_URL
    if model not in MODEL_URLS:
        raise ValueError(f"Unknown model '{model}'. Choose from: {list(MODEL_URLS)}")
    _ACTIVE_LLM_URL = MODEL_URLS[model]
    logger.info("LLM backend set to %s  (%s)", model, _ACTIVE_LLM_URL)
...
_LEXICON: Dict[str, Dict] = {
    "age": {
        "max_score": 15,
        "keywords": [
            "young", "old", "elderly", "senior", "child", "children", "adult",
            "teenager", "teen", "middle-aged", "infant", "toddler", "generation",
            "generations", "age", "aged", "youth", "elder",
        ],
    },
    "ethnic_racial": {
        "max_score": 20,
        "keywords": [
            "diverse", "multicultural", "multiracial", "ethnicity", "ethnic",
            "asian", "african", "latino", "hispanic", "middle eastern",
            "european", "indigenous", "aboriginal", "south asian",
            "east asian", "southeast asian", "black", "white", "brown",
            "mixed", "biracial", "multiethnic", "global", "international",
            "various backgrounds", "different backgrounds",
        ],
    },
    "gender": {
        "max_score": 15,
        "keywords": [
            "men", "women", "man", "woman", "male", "female", "non-binary",
            "nonbinary", "gender", "genderqueer", "transgender", "trans",
            "gender-inclusive", "gender-diverse", "they", "them",
            "inclusive", "all genders",
        ],
    },
    "cultural": {
        "max_score": 20,
        "keywords": [
            "cultural", "culture", "tradition", "traditional", "heritage",
            "customs", "practice", "practices", "attire", "clothing",
            "garment", "festival", "celebration", "ritual", "ceremony",
            "religious", "spiritual", "community", "communities",
            "background", "backgrounds", "diverse clothing",
        ],
    },
    "ability": {
        "max_score": 10,
        "keywords": [
            "disability", "disabilities", "wheelchair", "accessible",
            "accessibility", "hearing impaired", "visually impaired",
            "blind", "deaf", "prosthetic", "mobility", "neurodivergent",
            "diverse abilities", "differently abled", "inclusive",
        ],
    },
    "socioeconomic": {
        "max_score": 10,
        "keywords": [
            "background", "backgrounds", "community", "communities",
            "various", "different", "socioeconomic", "working class",
            "professional", "trade", "occupation", "diverse occupations",
            "mixed", "varied",
        ],
    },
    "specificity": {
...
_BIAS_WORDS: List[str] = [
    "housewife", "mankind", "manpower", "stewardess", "fireman",
    "policeman", "mailman", "chairman", "steward",
    "exotic", "oriental", "colored", "primitive",
    "senile", "crone", "hag",
    "crippled", "retarded", "insane", "crazy", "psycho",
]
...
def _keyword_result_to_score_dict(kw: _KeywordScoreResult) -> Dict:
    ...
    breakdown = {
        _DIM_KEY_MAP.get(d.name, d.name): d.score
...
def _log_score_result(result: Dict, label: str = "") -> None:
    ...
    kw_score = result.get("keyword_score", result.get("overall_score", "?") if method == "keyword_score" else "N/A")
    logger.info("%sOverall=%s/100  method=%s  keyword=%s/100  words=%s",
                tag, overall, method, kw_score, result.get("word_count", "?"))
    ...
    if bd:
        logger.info(
            ...
            tag,
            bd.get("age_diversity", "?"), bd.get("ethnic_racial_diversity", "?"),
            bd.get("gender_diversity", "?"), bd.get("cultural_diversity", "?"),
            bd.get("ability_inclusion", "?"), bd.get("socioeconomic_diversity", "?"),
            bd.get("specificity", "?"),
        )
    if result.get("bias_words_found"):
        logger.warning("%s  Bias words detected (-%d pts): %s",
                       tag, result.get("bias_penalty", 0),
                       ", ".join(result["bias_words_found"]))


def _qwen3_available() -> bool:
    """Return True if the active LLM server is up and ready."""
    try:
        r = requests.get(f"{_ACTIVE_LLM_URL}/health", timeout=5)
...
def _call_qwen3(
    ...
    """Send a prompt to the local Qwen3 server and return the text output."""
    payload = {
        "prompt": prompt,
    ...
    response = requests.post(f"{_ACTIVE_LLM_URL}/generate", json=payload, timeout=2400)
...
Examples:
  ...
  python enhance_prompt_sequential.py --threshold 80 --max-iterations 5 # Custom settings
    ...
    parser.add_argument(
        ...
        type=str,
        default="qwen3",
        choices=["qwen3", "qwen3.5"],
        help="Which local LLM server to use: 'qwen3' (port 8001, Qwen3-32B) or "
             "'qwen3.5' (port 8002, Qwen3.5-27B). Default: qwen3"
    )
...
def llm_score(
    ...
    **Always** runs keyword scoring first.  Then attempts the LLM judge
    ...
    logger.info(
        ...
        kw.final_score, kw.word_count, kw.bias_penalty,
    )
    if kw.bias_words_found:
        logger.warning(
            "Bias words detected in prompt (-%d pts): %s",
            kw.bias_penalty, ", ".join(kw.bias_words_found),
        )

    # ── 2. Decide whether to run the LLM judge ────────────────────────────────
    if not use_llm:
        logger.info("LLM judge skipped (use_llm=False) — keyword score is final")
        return kw_dict

    if not _qwen3_available():
        logger.warning("LLM judge server not reachable — using keyword score as final score")
        return kw_dict
    ...
    try:
        ...
        logger.debug("[llm_score] extracted JSON (first 400 chars): %r", score_text[:400])
        ...
        if "breakdown" in score_data and not isinstance(score_data.get("breakdown"), dict):
...
def get_llm_enhancement(enhancement_prompt: str, graphrag: GraphRAG) -> str:  # noqa: ARG001
    """Get enhancement from the local Qwen3 server."""
    logger.info("Generating enhanced prompt with local Qwen3...")

    if not _qwen3_available():
        logger.warning("Qwen3 server not reachable, using mock response")
        return get_mock_enhancement(enhancement_prompt)

    try:
        with log_timer(logger, "Prompt enhancement via Qwen3"):
            enhanced_result = _call_qwen3(enhancement_prompt, temperature=0.3, max_new_tokens=8192)
...
def sequential_enhance_prompt(
    ...
    logger.info("Starting sequential enhancement | threshold=%d | max_iterations=%d", threshold, max_iterations)
    ...
    for iteration in range(1, max_iterations + 1):
        ...
        logger.debug("[enhancement iter %d] raw LLM output (first 600 chars): %r",
                     iteration, enhanced_result[:600])
        if out_dir is not None:
            raw_dir = out_dir / "raw_llm_outputs"
            raw_dir.mkdir(parents=True, exist_ok=True)
            ts = datetime.now().strftime("%H%M%S_%f")
            ...
            logger.debug("[enhancement iter %d] raw output saved to %s", iteration, raw_path)
        ...
        if iteration_data["threshold_met"]:
            logger.info("Diversity threshold %d reached after %d iteration(s)", threshold, iteration)
            break
    ...
    if final_score >= threshold:
        logger.info(
            ...
            final_score, final_method, threshold,
        )
    else:
        logger.warning(
...
def display_final_results(original_prompt: str, final_prompt: str, iteration_history: List[Dict], 
    ...
    print(f"Score    : {final_ds['overall_score']}/100  (method: {final_ds.get('score_method', '?')})")
...
def write_output_csvs(
    ...
    prompts_fields = ["prompt", "__row_id__", "generic_prompt", "enhanced_prompts", "finalized_prompt"]
    with prompts_csv.open("w", newline="", encoding="utf-8") as f:
        ...
        for row in rows_data:
            ...
            best = _best_iteration(history)["enhanced_prompt"] if history else row["original_prompt"]
            writer.writerow({
    ...
    detail_fields = ["row_id", "generic_prompt", "generic_score", "generic_score_method"]
        ...
        for row in rows_data:
            ...
            for it in history:
                ...
                if n <= max_iters:
                    ...
                    record[f"score_{n}_method"]     = it["diversity_score"].get("score_method", "")
            ...
            if history:
                ...
                if row.get("eval_final_score") is not None:
                    ...
                    record["final_score_method"] = row.get("eval_final_score_method", "llm_score")
                else:
                    record["final_score"]        = best["diversity_score"]["overall_score"]
                    record["final_score_method"] = best["diversity_score"].get("score_method", "")
            else:
                record["final_prompt"]        = row["original_prompt"]
                record["final_score"]         = row.get("eval_final_score", row.get("original_score", ""))
                record["final_score_method"]  = row.get("eval_final_score_method", row.get("original_score_method", ""))
...
def main() -> None:
    ...
    # Route LLM calls to the chosen model server
    configure_llm(args.model)
    ...
    original_score_data   = llm_score(original_prompt, out_dir=out_dir)
    original_score        = original_score_data["overall_score"]
    original_score_method = original_score_data["score_method"]
    logger.info("Original prompt score: %d/100 (method: %s)", original_score, original_score_method)
    ...
    if original_score >= args.threshold:
        logger.info(
            ...
            original_score, original_score_method, args.threshold,
        )
