    ...
    with csv_path.open(encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            rows.append({
                "row_id": row_id,
                "generic_prompt": gp,
                "enhanced_prompts_csv": _parse_enhanced_prompts(row.get("enhanced_prompts", "")),
            })
    return rows
...
def _save_summary(results: List[Dict], out_dir: Path) -> None:
    ...
    # ── CSV ──────────────────────────────────────────────────────────────────
    fieldnames = [
        "row_id", "original_prompt", "enhanced_prompt",
        "keyword_original", "keyword_enhanced", "keyword_delta",
        "llm_original", "llm_enhanced", "llm_delta",
        "verdict",
    ]
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as f:
        ...
        for r in results:
            ...
            kw_delta = (kw_enh - kw_orig) if isinstance(kw_orig, int) and isinstance(kw_enh, int) else ""

            llm_orig  = orig.get("overall_score", "") if orig.get("score_method") == "llm_score" else ""
            llm_enh   = enh.get("overall_score", "")  if enh.get("score_method")  == "llm_score" else ""
            llm_delta = (llm_enh - llm_orig) if isinstance(llm_orig, int) and isinstance(llm_enh, int) else ""
...
Examples:
  ...
  python run_batch.py --input inputs/my.csv  # custom CSV
    ...
    parser.add_argument(
        "--input", default="inputs/prompts.csv",
    ...
    parser.add_argument(
        "--row-ids", nargs="+", type=int, default=None,
...
def main() -> None:
    ...
    if not args.no_enhance:
        ...
        if not bias_ok and not cultural_ok:
            logger.error("Failed to load any knowledge graphs — cannot enhance. Exiting.")
            sys.exit(1)
    ...
    for row in rows:
        ...
        with log_timer(logger, f"Scoring original — {label}"):
            orig_score_data = llm_score(original, out_dir=out_dir, use_llm=not args.no_llm)
        ...
        if args.no_enhance:
            ...
            if csv_enhanced:
                ...
                logger.warning("  Row %d has no enhanced_prompts in CSV; skipping enhancement", row_id)
            ...
            with log_timer(logger, f"Scoring final — {label}"):
                final_score_data = llm_score(enhanced_prompt, out_dir=out_dir, use_llm=not args.no_llm)
        else:
            if eval_orig_score >= args.threshold:
                logger.info(
                    "  Original score %d/100 (%s) already meets threshold %d — no enhancement needed",
                    eval_orig_score, eval_orig_method, args.threshold,
                )
                enhanced_prompt = original
                ...
                triples = retrieve_relevant_triples(
                    graphrag, original, args.bias_top_k, args.cultural_top_k,
                )
        ...
        logger.info(
            ...
            eval_orig_score, eval_orig_method,
            eval_enh_score,  eval_enh_method,
            score_delta, verdict,
        )
        ...
        logger.info(
            ...
            eval_orig_score, eval_enh_score, score_delta, verdict,
        )
    ...
    logger.info("  (no evaluation/ folder; scores are in enhanced/*.json and summary.csv)")
    logger.info("  prompts_output.csv  ← mirrors inputs CSV with finalized_prompt column")
