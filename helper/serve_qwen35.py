#!/usr/bin/env python3
"""FastAPI server for local inference with Qwen/Qwen3.5-27B.

This server wraps the Qwen3.5-27B causal language model as a REST API.
...
Model vs. serve_qwen3.py:
    - Model:  Qwen/Qwen3.5-27B  (this server, port 8002)
              Qwen/Qwen3-32B    (serve_qwen3.py,  port 8001)
    - Qwen3.5-27B offers an improved instruction-following capability and
      stronger multilingual & reasoning performance in a smaller 27 B footprint.
    - Approximately ~54 GB VRAM required in bfloat16; falls back to float32 on CPU.
...
Notes:
    ...
    - Run the pipeline scripts with --model qwen3.5 to route requests here.
"""
...
def _build_input_ids(prompt: str) -> dict:
    """Build tokenized inputs using the chat template so Qwen3.5 thinking works correctly."""
...
async def lifespan(_app: FastAPI):
    ...
    _state["ready"] = True
    logger.info("Qwen3.5 server is ready")
    yield
    logger.info("Qwen3.5 server shutting down")


app = FastAPI(title="Qwen3.5 Local Text Generation Server", lifespan=lifespan)
...
async def generate(request: GenerateRequest) -> GenerateResponse:
    ...
    async with _generation_lock:
        ...
        try:
            with log_timer(logger, "Qwen3.5 generation"):
